...
export async function initEmployeeManager() {
    ...
    const renderTable = (data) => {
        ...
        tableBody.innerHTML = data.map(nv => {
            const dateFormatted = nv.NgayTao ? new Date(nv.NgayTao).toLocaleDateString('vi-VN') : 'N/A';
            return `
                <tr>
    ...
    employeeForm.onsubmit = async (e) => {
        ...
        try {
            if (currentEditId) {
                ...
                await axios.put(`http://localhost:3000/api/employees/${currentEditId}`, baseData);
                ...
                await axios.post('http://localhost:3000/api/employees', createData);
            ...
            if (err.response) {
                ...
                errorMsg = "Hệ thống không nhận được phản hồi từ server. Vui lòng kiểm tra lại Node.js backend!";
            }
            ...
            Swal.fire({
                ...
                title: 'Thất bại',
                text: errorMsg,
                footer: '<small class="text-muted">Mẹo: Nhấn F12, xem tab Console/Network để biết chi tiết câu lệnh lỗi.</small>'
            });
        }
    };
    ...
    tableBody.onclick = async (e) => {
        ...
        if (btnToggle) {
            ...
            const isCurrentActive = btnToggle.getAttribute('data-status') === 'true' || btnToggle.getAttribute('data-status') === '1';
            const targetStatus = !isCurrentActive;

            const confirmResult = await Swal.fire({
                title: isCurrentActive ? 'Xác nhận khóa tài khoản?' : 'Kích hoạt lại tài khoản?',
                text: isCurrentActive ? "Nhân viên bị khóa sẽ không thể đăng nhập vào hệ thống!" : "Nhân viên sẽ được khôi phục quyền thao tác!",
                icon: 'warning',
            ...
            if (confirmResult.isConfirmed) {
                try {
                    await axios.patch(`http://localhost:3000/api/employees/${id}/toggle-status`, { TrangThai: targetStatus });
                    Swal.fire('Thành công!', targetStatus ? 'Đã kích hoạt lại tài khoản.' : 'Đã khóa tài khoản thành công.', 'success');
                    loadEmployees();
                } catch (err) {
                    Swal.fire('Thất bại', err.response?.data?.message || "Không thể thực thi lệnh", 'error');
                }
            }
        }
    };
